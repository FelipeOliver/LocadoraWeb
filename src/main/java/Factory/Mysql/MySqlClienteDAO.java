...
public class MySqlClienteDAO implements ClienteDAO {
	...
	public MySqlClienteDAO(Connection c){
	   ...
	   public String cadastraCliente( Cliente cliente ) {
	      try {
	         String sql = "insert into Cliente (idCpf, nome, rg, telefone, email, dtNascimento, sexo, numRegistro, estado, validadeCNH, dataEmissaoCNH) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";       
	   ...
	   public Cliente consultaCliente( String cpf ) {
	      ...
	      try {
	         String sql = "select * from Cliente where idCpf = ?";
	         ...
	         if ( resultSet.next() ) { // so espero um resultado por isso uso o IF para verificar
	            ...
	            cliente.setCpf( resultSet.getString( "idCpf" ) );	// coloca-se os dados
	      ...
	      try
			{
	         ...
	         JOptionPane.showMessageDialog( null, "N�o existe cadastro para o nome informado", "ERRO", JOptionPane.ERROR_MESSAGE );
	      }
	      return cliente;
	   }
	   ...
	   public void alteraCliente( Cliente cliente ) { // recebe o bean com os dados a serem alterados no banco 
	      try {
	         String sql = "update Cliente set nome = ?, rg = ?, telefone = ?, email = ?, dtNascimento = ?, sexo = ?, numRegistro = ?, estado = ?, validadeCNH = ?, dataEmissaoCNH = ? where idCpf = ?";
	         st = conn.prepareStatement( sql ); // prepara o statement
	         //st.setString( 1, cliente.getCpf() ); // substitui as ? por valores
	   ...
	   public void excluiCliente( String cpf ) { //delete registro pelo Codigo 
	      try {
	         String sql = "delete from Cliente where idCpf = ?";
	         st = conn.prepareStatement( sql ); // prepara o statement
	         ...
	         JOptionPane.showMessageDialog( null, "Nao existe cadastro para o cpf informado", "ERRO", JOptionPane.ERROR_MESSAGE );
	      } 
	   }
	   ...
	   /*public ArrayList<FornecedorTO> listarItens()	// o retorno  um ArrayList generics de ItemsFornecedor
		{ 
	      ...
	      try
			{
	         String sql = "select * from projeto.tabFornecedor order by frnNome"; // ordena por ordem alfabetica ascendente a-z
	         st = conn.prepareStatement( sql );
