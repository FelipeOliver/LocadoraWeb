...
public class PgAutomovelDAO implements AutomovelDAO {
	private PreparedStatement st; //prepara a consulta sql
	Connection conn = null;
	
	public PgAutomovelDAO(Connection c){
		conn = c;
	}

	
	// mï¿½todo para incluir registros no banco de dados
		public void cadastraAutomovel( Automovel automovel ) {
			try {
				//String paSql = "set Cliente_encoding = 'UTF8'";
				 //conn.prepareStatement( paSql ).executeUpdate();
		   ...
		   public AutomovelTO consultaPorNome( String nome )
			{
		      ...
		      try
				{
		         String sql = "select * from Automovel where upper(modelo) like upper(?) and statusAutomovel <> ?"; // like permite encontrar nomes com coringas
		         st = conn.prepareStatement( sql );
		      ...
		      try
				{
		         ...
		         ResultSet resultSet = st.executeQuery();
		         while( resultSet.next() ) {
		            String resp = ""; // crio uma instï¿½ncia 
		   ...
		   public ArrayList<Automovel> consultaAutomovelEstado( String estado ) {
			   ArrayList<Automovel> automoveis = new ArrayList<Automovel>();
			      try {
			         ...
			         while ( resultSet.next() ) { // s� espero um resultado por isso uso o IF para verificar
